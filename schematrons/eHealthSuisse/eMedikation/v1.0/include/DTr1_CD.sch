...
<rule xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:xs="http://www.w3.org/2001/XMLSchema" abstract="true" id="CD">
    ...
    <assert role="error" test="not(@nullFlavor and @code)">dtr1-6-CD: co-occurence violation. Cannot have code and null</assert>
    <assert role="error" test="not(@nullFlavor and @displayName)">dtr1-7-CD: co-occurence violation. Cannot have displayName and null</assert>
    ...
    <assert role="error" test="not(.//hl7:translation[@codeSystemName][not(@codeSystem)])">dtr1-12-CD: translation codeSystemName only if codeSystem</assert>
    <assert role="error" test="not(.//hl7:translation[@codeSystemVersion][not(@codeSystem)])">dtr1-13-CD: translation codeSystemVersion only if codeSystem</assert>
    ...
    <assert role="error" test="not($set_other[@code=$code][@codeSystem=$codeSystem])">dtr1-18-CD: [RIM-002] <value-of select="$name"/> (@code=<value-of select="$code"/> @codeSystem=<value-of select="$codeSystem"/>) SHALL be distinct</assert>
    ...
    <assert role="error" test="not(.//hl7:originalText[@language and not(matches(@language,$RFC3066,'i'))])">dtr1-20-CD: originalText/@language SHALL conform to RFC 3066. This usually has format sss?-CC with sss? for language code (ISO-639-1 / ISO-639-2) and optional CC for country code (conform ISO-3166 alpha-2)</assert>
</rule>